...
public class AgentMepToolController {
    ...
    public AgentContract.ToolResponse<?> deployment(
        ...
        if (competitionLifecycleService.supports(body.arguments().deploymentUid())) {
            java.util.Map<String, Object> data = competitionLifecycleService.deploymentEvidence(context, body.arguments().deploymentUid());
            return AgentContract.success(
                    data, context, "XnetMLOps/competition-deployment", String.valueOf(data.get("resourceVersion")), startedNanos);
        }
    ...
    public AgentContract.ToolResponse<?> probe(
        ...
        if (body.arguments() != null && competitionLifecycleService.supports(body.arguments().deploymentUid())) {
            ...
            return AgentContract.success(
                    data, context, "XnetMLOps/competition-probe", String.valueOf(data.get("resourceVersion")), startedNanos);
        }
